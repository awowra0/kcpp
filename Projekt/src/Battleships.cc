...
void Battleships::screene(int i){//clears whole screen, prints only text, used twice
	clear();
	if (i){
...
void Battleships::screen(board *b, int kk){//prints player board during placement, kk-ship id
	clear();
	move(0,0);
...
void Battleships::screend(board *b, board *bb, int code = 0){//prints ingame boards; b - player board, bb - player shooting board, code - game state
	clear();
	move(0,0);
	char ctemp[message.length()+1];
	char csingle[1];
...
bool Battleships::checkpoint(board *b, int x, int y){//checks x,y position whether it is fine to place
...
bool Battleships::singleplacement(board *b, int k[4][2], int toplace, int kk){//tries to place ship containing first position, b - 	board, k - ship coordinates, toplace - ship size, kk - ship id
...
void Battleships::com_placement(board *b){//random ship placement
...
bool Battleships::psplacement(board *b, int size, int kk){//manual ship placement. b - player board, size - ship size, kk - ship id
	int c;
...
void Battleships::my_placement(board *b){//controlls manual placement. b - player board
...
void Battleships::shipsink(board *b, int id, board *bb){//ship sinking procedure. b - victim board , id - ship to sink, bb - attacker shooter board
...
void Battleships::shiphit(board *b, board *bb){//b - victim board , bb - attacker shooter board
...
bool Battleships::nextfrepeat(int x, int y){//checks x,y position for comp tactic2 next possible position to cross-attack; prevents writing two same positions to attack
	for (int i=0; i<ai.nextf; i++){
		if (ai.nextfield[i][0] == x && ai.nextfield[i][1] == y) return false;
	}
	return true;
}
void Battleships::compmove(board *b, board *bb){//b - comp shooting board, bb - player board
...
void Battleships::clearboard(board *b){//clears selected board by changing all positions to = (empty/unknown) 
	fill(b->fields[0], b->fields[0]+100, '=');
}
void Battleships::selectopt(board *b){//pre-game options. b - player board, i - build board parameter
	char c;
	screene(0);
	while(1){
		c = getch();
...
char Battleships::endgame(int e, board *myb, board *sb, board *cpb, board *csb){//handles endgame sequence - new game or leaving program
	char c;
	screend(myb, cpb, e);
	cursor.x = 0;
	cursor.y = 0;
...
int Battleships::run(){//'main' function, handles game and inits variables
